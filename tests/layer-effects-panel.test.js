...
test("mobile adjustment layer exposes implemented effects as bottom toolbar panels", () => {
  ...
  const appSource = fs.readFileSync(path.join(repoRoot, "js", "app.js"), "utf8");
  const layerStyleIndex = indexSource.indexOf("mobile-layer-style-button");
  const aiIndex = indexSource.indexOf("data-ai-board-menu-trigger");

  assert.match(indexSource, /class="tool-button mobile-adjustment-layer-button"[\s\S]*data-tool-mode="adjustments"/);
  assert.match(indexSource, /class="tool-button mobile-layer-style-button"[\s\S]*aria-label="LAYER STYLE"[\s\S]*data-tool-mode="layer-style"/);
  assert.ok(layerStyleIndex >= 0);
  assert.ok(aiIndex >= 0);
  assert.ok(layerStyleIndex < aiIndex);
  assert.match(toolbarCss, /\.mobile-adjustment-layer-button[\s\S]*display: none;/);
  assert.match(toolbarCss, /\.mobile-layer-style-button[\s\S]*display: none;/);
  assert.match(toolbarCss, /\.toolbar-dock \.mobile-adjustment-layer-button,[\s\S]*\.toolbar-dock \.mobile-layer-style-button \{\s*display: inline-flex;/);
  ...
  assert.match(source, /data-mobile-layer-effect-mode="\$\{effect\.mobileEffectMode\}"/);
  ...
  assert.match(source, /data-mobile-layer-effects-editor="color-overlay"/);
  assert.match(source, /data-mobile-layer-effects-editor="stroke"/);
  ...
  assert.match(source, /label === "LAYER STYLE" \|\| toolMode === "layer-style"/);
  assert.match(source, /setMobileLayerEffectsMode\(isLayerStyle \? "style" : "adjustments"\)/);
  assert.match(source, /effectButton\.hidden = effectMode !== activeMobileLayerEffectsMode/);
  ...
  assert.match(source, /source: "mobile-layer-effects-color-overlay"/);
  assert.match(source, /source: "mobile-layer-effects-stroke"/);
