...
function readBrushEngineSources() {
  return brushEngineModulePaths.map((parts) => readRepoFile(...parts)).join("\n");
}
...
test("rect area selection is loaded and wired to the side toolbar", () => {
  ...
  assert.match(selectionSource, /const MAGIC_WAND_TOOL_MODE = "selection-magic-wand"/);
  ...
  assert.match(selectionSource, /function createMagicWandRegion\(source, seedPoint, tolerance, bounds = getDocumentRect\(\)\)/);
  assert.match(selectionSource, /function selectMagicWandAt\(point\)/);
  assert.match(selectionSource, /source: "area-selection-magic-wand-commit"/);
  assert.match(selectionSource, /getMagicWandPixel\(source, documentX, documentY\)/);
  ...
  assert.match(selectionSource, /selectMagicWandAt,/);
  ...
  assert.match(topToolbarSource, /toolMode === "selection-magic-wand"/);
  ...
  assert.match(topToolbarSource, /showColorRangeToleranceControl\(isColorRange \|\| isMagicWand\)/);
...
test("rect selection drag supports shift square and alt center modifiers", () => {
  ...
  assert.match(selectionSource, /function isMagicWandToolActive\(\) \{/);
  assert.match(selectionSource, /state\.activeToolMode === MAGIC_WAND_TOOL_MODE/);
  ...
  assert.match(selectionSource, /isMagicWandToolActive\(\)/);
