...
test("brush engine keeps shape textures alpha-only without mipmaps", () => {
  ...
  assert.match(source, /float coreShape = sampleShapeTextureAlpha\(v_uv\);/);
  assert.match(source, /float rightShape = sampleShapeTextureAlpha\(v_uv \+ vec2\(offset, 0\.0\)\);/);
  assert.match(source, /textureSize\(u_shapeTexture, 0\)/);
  assert.match(source, /texelFetch\(u_shapeTexture, texel, 0\)\.a/);
