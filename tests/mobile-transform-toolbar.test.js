...
test("mobile transform sidebar exposes resize rotate distortion perspective and puppet", () => {
  ...
  assert.equal((resizeButtonSource.match(/class="tool-button mobile-transform-tool-button"/g) || []).length, 5);
  assert.match(resizeButtonSource, /aria-label="RESIZE"[\s\S]*data-transform-select-mode="free"[\s\S]*data-tool-mode="resize"/);
