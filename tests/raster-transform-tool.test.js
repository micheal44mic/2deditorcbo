...
test("document renderer supports raster transform preview and history commit", () => {
  ...
  assert.match(source, /float clipAlpha = sampleClipAlpha\(v_destPixel\)/);
...
test("raster transform supports Photoshop-style warp mesh preview and commit", () => {
  ...
  assert.match(toolSource, /const WARP_POINT_TOUCH_HIT_RADIUS_PX = 38;/);
...
test("raster transform tool uses SVG overlay, resize/rotate activation, and history-safe commit", () => {
  ...
  assert.match(source, /function isAndroidFastResizeBoundsEnabled\(\)/);
  assert.match(source, /function isAndroidLiveTransformPreviewEnabled\(\)/);
  ...
  assert.match(source, /const MOBILE_HANDLE_SIZE = 18;/);
  assert.match(source, /const HANDLE_TOUCH_HIT_RADIUS_PX = 34;/);
  ...
  assert.match(source, /getHandleHitAtClient\(clientX, clientY, pointerType = ""\)/);
  assert.match(source, /shouldStartOffArtboardLayerDrag\(point, pointerType = ""\)/);
  assert.match(source, /return this\.isPointInCurrentQuad\(point\)/);
  ...
  assert.match(source, /getCoarseRasterContentBounds\(layerOrId\)/);
  assert.match(source, /getPixelTightRasterContentBounds\(layerOrId\)/);
  assert.match(source, /isAndroidFastResizeBoundsEnabled\(\)/);
  assert.match(source, /this\.documentRenderer\?\.getRasterContentBounds\?\.\(layerId, getRasterTransformBoundsOptions\(\)\)/);
  assert.match(source, /const bounds = this\.getPixelTightRasterContentBounds\(layer\)/);
  assert.match(source, /this\.documentRenderer\?\.width/);
  assert.match(source, /this\.documentRenderer\?\.height/);
  assert.match(source, /getLiveRasterTransformPreview\(effectiveTransformMode = this\.getEffectiveTransformMode\(\)\)/);
  assert.match(source, /liveTexture: true/);
  assert.match(source, /sourceUvRect: livePreview\.sourceUvRect/);
  ...
  assert.match(source, /const translateOnly = this\.documentRenderer\?\.getTranslateOnlyRasterTransformDelta\?\.\(\{/);
  assert.match(source, /const sourceSnapshot = translateOnly[\s\S]*\? this\.sourceSnapshot[\s\S]*: this\.sourceSnapshot \|\| this\.createSourceSnapshot\(\);/);
  ...
  assert.match(source, /this\.getHandleTarget\(event\.target\) \|\|[\s\S]*this\.getHandleHitAtClient\(event\.clientX, event\.clientY, event\.pointerType\)/);
  assert.match(source, /const offArtboardMove = !handle &&[\s\S]*this\.shouldStartOffArtboardLayerDrag\(point, event\.pointerType\)/);
  assert.match(source, /\|\| \(offArtboardMove \? this\.box : null\)/);
  assert.match(source, /const mode = offArtboardMove[\s\S]*\? "move"[\s\S]*: this\.isRotateActive\(\)/);
  ...
  assert.match(source, /if \(this\.isActive\(\) && !this\.isWarpMode\(\)\) \{\s*const handlePoints = this\.getHandlePoints\(points\);[\s\S]*const handleSize = this\.getHandleSize\(\);[\s\S]*setSvgElementVisible\(handle, true\);/);
