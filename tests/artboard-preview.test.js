...
test("artboard connections live in their own module", () => {
  ...
  assert.match(connectionsSource, /element\.classList\.toggle\("is-generating", isHeavyMounted && aiImageGeneratingBoardIds\.has\(board\.id\)\)/);
  assert.match(connectionsSource, /setStylePropertyIfChanged\(element, "transform", `translate3d\(\$\{Number\(board\.x\) \|\| 0\}px, \$\{Number\(board\.y\) \|\| 0\}px, 0\)`\)/);
