...
test("artboard connections live in their own module", () => {
  ...
  assert.match(connectionsSource, /const shouldDeferPreviewWork = cameraMotionActive && !isGenerating && !isFocusedOrSelected/);
  ...
  assert.match(connectionsSource, /dimensions\.textContent = `\$\{Math\.round\(docWidth\)\} \\u00d7 \$\{Math\.round\(docHeight\)\}`/);
  ...
  assert.match(connectionsSource, /const canEditCaption = isSelected/);
  assert.match(connectionsSource, /const shouldShow = isSelected \|\| hasCaption/);
  ...
  assert.match(cssSource, /\.editor-ai-image-board\.is-selected \.editor-ai-image-board-caption-editor:focus[\s\S]*box-shadow: 0 0 0 var\(--ai-caption-focus-ring-width, 1px\)/);
